        ...
        <div class="row">
            ...
            <div class=" col-12 mb-3">
                ...
                <div class="form-group mb-2 mb20">
                    <label for="por-municipio" class="form-label text-info">Municipios<span class="text-danger">*</span></label>
                    <select name="por-municipio" class="form-control mb-2 mb20" id="por-municipio">
                        <option value="" disabled selected>Selecciona un Municipio</option>
                        @foreach($municipios as $municipio)
                        <option value="{{ $municipio->id }}">
                            {{ $municipio->nombre }} - {{ $municipio->provincia }}
                        </option>
                        @endforeach
                    </select>
...
<script>
    $(document).ready(function() {
        ...
        $('#por-municipio').on('change', function() {

            const selectedValue = $(this).val(); // Obtener el valor seleccionado
            // console.log(`Municipio seleccionado: ${selectedValue}`);
        ...
        function handleTipoTransmisionChange(value) {
            ...
            $.ajax({
                url: "{{ route('reportes-municipio.reporteMunicipioPost') }}",
                method: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    municipio_id: value
                ...
                success: function(response) {
                    ...
                    // Si no hay informes, mostrar un mensaje en la tabla
                    if (!informes || informes.length === 0) {
                        datosHtml = '<tr><td colspan="7" class="text-center">No hay informes disponibles para este municipio.</td></tr>';
                        $('#tbody-municipio').html(datosHtml);
                        return;
                    }
